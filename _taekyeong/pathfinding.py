...
def _build_grid():
    ...
    # 도로 + 횡단보도 진입 통로를 장애물로 처리 (y=1.0~7.5).
    # 진입 구역을 도로보다 1.5m 남쪽(y=1.0)부터 막아
    # A* 경로가 최소 2m 이상 x=0 북향 직진으로 횡단보도에 접근하도록 강제.
    CW_OPEN = 1.0          # 횡단보도 통로 반폭 (m) — 흰색 줄 폭 1.2m에 맞춤
    gy0 = max(0, _w2g(0, 1.0)[1])   # ← 2.5 → 1.0 으로 연장 (직진 진입 강제)
    gy1 = min(GRID_SIZE-1, _w2g(0, 7.5)[1])
